		...
		>
			<div>
				...
				<ul class="menu space-y-5">
					{#each MainRoute as route (route.path)}
						{#if route.icon}
							{@const Icon = route.icon}
						<li>
							<a href={route.path} class="flex w-full items-center gap-2">
								<Icon class="h-5 w-5" />
								{route.name}
							</a>
						</li>
			...
			<div class="p-4">
				<form method="post" action="/main/Therapists/signout">
					<button class="btn btn-ghost btn-error w-full" title="Sign out">Sign Out</button>
				</form>
			</div>
		</aside>
	</div>
